            ...
            const uint8_t *buf)
{
    ...
    if (len <= sizeof(tf.single.data)) {
        ...
        tp_tx.resid = 0;
        tp_tx.sequence = 0;
        HAL_timer_reset(tp_tx.timeout, 0);
        tp_tx.buf = NULL;
    }
    ...
    // otherwise pack a first frame and set up to continue
        ...
        tp_tx.sequence = 1;
        HAL_timer_register(tp_tx.timeout);
        HAL_timer_reset(tp_tx.timeout, timeout_ms);
    ...
    HAL_can_send_blocking(0x600 + ISO_TP_NODE_ID, 8, &(tf.raw[0]));
...
iso_tp_tx_send_next(void)
{
    ...
    if ((iso_tp_send_done() == ISO_TP_BUSY) &&
        ...
        HAL_can_send_blocking(0x600 + ISO_TP_NODE_ID, 8, &(tf.raw[0]));
        tp_tx.window_resid--;
        HAL_timer_reset(tp_tx.interval, tp_tx.interval_ms);
    }
}
...
iso_tp_send_done()
{
    ...
    if (!HAL_timer_expired(tp_tx.timeout)) {
        return ISO_TP_BUSY;
    }
            ...
            uint8_t *buf)
{
    ...
    tp_rx.resid = len;
    tp_rx.buf = buf;
    HAL_timer_register(tp_rx.timeout);
    HAL_timer_reset(tp_rx.timeout, timeout_ms);
}
...
iso_tp_recv_done()
{
    ...
    // outstanding data and not timed out? must be busy
    if (!HAL_timer_expired(tp_rx.timeout)) {
        return ISO_TP_BUSY;
    }
...
iso_tp_rx_flow(uint8_t sender, const tp_flow_t *tf)
{
    if ((iso_tp_send_done() == ISO_TP_BUSY) &&
        ...
        HAL_timer_register(tp_tx.interval);
        HAL_timer_reset(tp_tx.interval, 0);
        iso_tp_tx_send_next();
    }
}
...
iso_tp_rx_first(uint8_t sender, const tp_first_t *tf)
{
    ...
    if ((iso_tp_recv_done() == ISO_TP_BUSY) &&
        ...
        HAL_can_send_blocking(0x600 + ISO_TP_NODE_ID, 8, &(ff.raw[0]));
    }
}
...
//
bool
iso_tp_can_rx(const HAL_can_message_t *msg)
{
    tp_frame_t *tf = (tp_frame_t *)&msg->data[0];
    uint8_t sender_id = msg->id & 0xff;

    // ignore non-ISO-TP frames
    if ((msg->id < 0x600) || (msg->id > 0x6f0)) {
        return false;
    }

    // only consider frames intended for this node
    if (tf->header.recipient == ISO_TP_NODE_ID) {
        switch (tf->header.type) {
        // is this a flow-control message to the current sender, and
        // are they waiting for one?
        case TP_FLOW_CONTROL_FRAME:
            iso_tp_rx_flow(sender_id, &tf->flow);
            break;

        // is this a single frame aimed at the current recipient?
        case TP_SINGLE_FRAME:
