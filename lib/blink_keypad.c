...
static uint8_t          _keypad_id = _BK_DEFAULT_KEYPAD_ID;
...
static void             _bk_tick(void);
static HAL_timer_call_t _bk_tick_call = {
    _bk_tick,
    BK_TICK_PERIOD_MS,
    BK_TICK_PERIOD_MS
};

static HAL_timer_t      _bk_idle_timer;
...
bk_can_receive(const HAL_can_message_t *msg)
{
    ...
    if ((msg->id == (0x180 + _keypad_id))
        ...
        HAL_timer_reset(_bk_idle_timer, BK_IDLE_TIMEOUT_MS);
        return true;
    }
...
PT_DEFINE(blink_keypad)
{
    ...
    HAL_timer_call_register(_bk_tick_call);
    HAL_timer_register(_bk_idle_timer);
    HAL_timer_register(blink_timer);
    ...
    for (;;) {
        ...
        do {
            ...
            if (_keypad_id == 0xff) {
                ...
                HAL_can_send_debug(0x00, sizeof(bk_reset_all), bk_reset_all);
                continue;
            }
            ...
            //
            {
                ...
                for (i = 0; i < (sizeof(bk_init_messages) / 8); i++) {
                    HAL_can_send_debug(0x600 + _keypad_id, 8, &bk_init_messages[i][0]);
                    pt_yield(pt);
                }
            }
        ...
        for (;;) {
            ...
            // requested.
            pt_wait(pt, HAL_timer_expired(blink_timer) || (_update_flags & _UPDATE_KEYS));
            _update_flags = 0;

            if (HAL_timer_expired(blink_timer)) {
                HAL_timer_reset(blink_timer, BK_BLINK_PERIOD_MS);
            ...
            if (HAL_timer_expired(_bk_idle_timer)) {
                uint8_t i;
